...
peft_model_id = "/root/autodl-tmp/PiVe/unified_verifier/flan-t5-xxl-lora-verifier/checkpoint-1498"
config = PeftConfig.from_pretrained(peft_model_id)

# load base LLM model and tokenizer
print(f"Type: ", type(config.base_model_name_or_path))
...
model = AutoModelForSeq2SeqLM.from_pretrained(config.base_model_name_or_path, torch_dtype=torch.float16, device_map="cuda:0")
...
# Load the Lora model
model = PeftModel.from_pretrained(model, peft_model_id, torch_dtype=torch.float16, device_map="cuda:0")
model.eval()
...
if gradio:
    ...
    dataset_path = "/root/autodl-tmp/PiVe/prompt_scripts/GPT3.5_result_KELM/Iteration1/test_generated_graphs.txt"
    with open(dataset_path, 'r') as f:
        samples = f.readlines()
        with open('output/verifier_output.txt', 'a') as output_file:
            for sample in tqdm(samples):
                output = evaluate(sample, "")
                output_file.write(str(output).strip() + '\n')
